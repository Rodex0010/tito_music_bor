...
# - Translation files are stored in tito/locales/ as JSON files (en.json, si.json)
...
from tito import db, logger
from tito.core.emojis import apply_custom_emojis

# supported languages and their display names
# NOTE: "en" (English) is the bot's default language - see db.get_lang() / core/mongo.py
lang_codes = {
    "ar": "🇸🇦 العربية",
    "en": "🇺🇸 English",
    "si": "🇱🇰 සිංහල",
...
class LangDict(dict):
    # dictionary with a fallback for missing keys + automatic custom-emoji swap
    ...
    def __getitem__(self, key):
        try:
            ...
            if not val:  # use the fallback if the value is empty
                val = self.fallback.get(key, key)
        except KeyError:
            val = self.fallback.get(key, key)
        return apply_custom_emojis(val) if isinstance(val, str) else val
...
class Language:
    ...
    def __init__(self):
        ...
        self.lang_dir = Path("tito/locales")
    ...
    async def get_lang(self, chat_id: int) -> dict:
        ...
        # always wrap in LangDict so custom emoji + fallback logic is applied
        # consistently, no matter which language is active.

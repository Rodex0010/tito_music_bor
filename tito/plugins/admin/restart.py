...
from tito import app, db, lang, stop


@app.on_message(filters.command(["السجلات", "logs"], prefixes=["", "/"]) & app.sudo_filter)
@lang.language()
...
@app.on_message(filters.command(["المسجل", "logger"], prefixes=["", "/"]) & app.sudo_filter)
@lang.language()
async def _logger(_, m: types.Message):
    ...
    if m.command[1] not in ("on", "off", "تشغيل", "ايقاف"):
        return await m.reply_text(m.lang["logger_usage"].format(m.command[0]))

    if m.command[1] in ("on", "تشغيل"):
...
@app.on_message(filters.command(["اعادة_تشغيل"], prefixes=["", "/"]) & app.sudo_filter)
@lang.language()
async def _restart(_, m: types.Message):
    ...
    os.execl(sys.executable, sys.executable, "-m", "tito")

...
from tito import app, db, lang, logger, userbot, config
from tito.helpers import utils


@app.on_message(filters.command(["غادر", "leave"], prefixes=["", "/"]) & app.sudo_filter)
@lang.language()
async def _leave(_, m: types.Message):
    ...
    chat_id = m.chat.id
    chat_name = utils.esc(m.chat.title) or "this chat"
...
@app.on_message(filters.command(["غادر_الكل", "leaveall"], prefixes=["", "/"]) & app.sudo_filter)
@lang.language()

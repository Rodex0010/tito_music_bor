...
@app.on_message(filters.command(["انهاء", "وقف", "اسكت", "stop", "end"], prefixes=["", "/"]) & (filters.group | filters.channel) & ~app.bl_users)
@lang.language()
@can_manage_vc
async def _stop(_, m: types.Message):
    ...
    try:
        sent_msg = await m.reply_text(m.lang["play_stopped"].format(utils.actor_mention(m)))
    except (ChatSendPlainForbidden, ChatWriteForbidden):

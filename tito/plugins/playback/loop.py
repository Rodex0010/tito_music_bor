...
from tito.helpers import can_manage_vc


@app.on_message(filters.command(["تكرار", "loop"], prefixes=["", "/"]) & (filters.group | filters.channel) & ~app.bl_users)
@lang.language()
@can_manage_vc
async def _loop(_, m: types.Message):
    ...
    if len(m.command) > 1:
        ...
        elif mode_arg in ["single", "1", "one", "مقطع", "واحد"]:
            new_loop = 1

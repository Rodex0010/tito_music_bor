...
from tito import tune, app, db, lang, queue
from tito.helpers import can_manage_vc, utils


@app.on_message(filters.command(["تقديم", "ترجيع", "forward", "rewind", "back"], prefixes=["", "/"]) & (filters.group | filters.channel) & ~app.bl_users)
@lang.language()
@can_manage_vc
async def _seek(_, m: types.Message):
    ...
    if m.command[0].lower() in ("ترجيع", "rewind", "back"):
    ...
    if success:
        await sent.edit_text(
            m.lang["play_seeked"].format(stype, start_from, utils.actor_mention(m))
        )
    else:

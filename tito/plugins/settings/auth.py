...
from tito.helpers import admin_check, is_admin, utils


@app.on_message(filters.command(["تصريح", "الغاء_تصريح", "auth", "unauth"], prefixes=["", "/"]) & (filters.group | filters.channel) & ~app.bl_users)
@lang.language()
@admin_check
...
@app.on_message(filters.command(["قائمة_التصريح", "authlist"], prefixes=["", "/"]) & (filters.group | filters.channel) & ~app.bl_users)
@lang.language()
@admin_check
...
@app.on_message(filters.command(["تحديث_الكاش", "تحديث"], prefixes=["", "/"]) & (filters.group | filters.channel) & ~app.bl_users)
@lang.language()

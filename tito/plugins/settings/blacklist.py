...
@app.on_message(filters.command(["حظر_مجموعة", "blacklistchat"], prefixes=["", "/"]) & app.sudo_filter)
@lang.language()
async def _blacklist_chat(_, m: types.Message):
    ...
    if chat_id in db.blacklisted:
        return await m.reply_text(
            f"<blockquote>⚠️ {utils.esc(chat.title)} is already blacklisted</blockquote>"
        )

    await db.add_blacklist(chat_id)
    await m.reply_text(
        f"<blockquote><u><b>✅ ᴄʜᴀᴛ ʙʟᴀᴄᴋʟɪꜱᴛᴇᴅ</b></u>\n\n"
        f"<b>ᴄʜᴀᴛ:</b> {utils.esc(chat.title)}\n"
        f"<b>ɪᴅ:</b> <code>{chat_id}</code></blockquote>"
    )


@app.on_message(filters.command(["فك_حظر_مجموعة", "الغاء_حظر_مجموعة", "whitelistchat"], prefixes=["", "/"]) & app.sudo_filter)
@lang.language()
...
@app.on_message(filters.command(["المجموعات_المحظورة", "قائمة_الحظر", "blacklistedchats"], prefixes=["", "/"]) & app.sudo_filter)
@lang.language()
...
@app.on_message(filters.command(["حظر", "block"], prefixes=["", "/"]) & app.sudo_filter)
@lang.language()
...
@app.on_message(filters.command(["فك_حظر", "unblock"], prefixes=["", "/"]) & app.sudo_filter)
@lang.language()
...
@app.on_message(filters.command(["المحظورين", "قائمة_المحظورين", "blocked"], prefixes=["", "/"]) & app.sudo_filter)
@lang.language()

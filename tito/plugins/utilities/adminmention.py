...
@app.on_message(filters.command(["ادمن", "الادمن", "تبليغ", "admin", "mention"], prefixes=["", "/"]) & (filters.group | filters.channel))
@lang.language()
async def tag_admins(client, message: types.Message):
    try:
        # Extract the message without the trigger
        message_text = message.text or message.caption or ""
        cleaned_text = utils.esc(TRIGGER_PATTERN.sub("", message_text).strip())

        # Get user info (handle anonymous admins)
        sender = message.from_user
        if sender:
            user_display = utils.esc(sender.first_name)
            if sender.username:
        ...
        try:
            async for admin in app.get_chat_members(
                ...
                if user.username:
                    ...
                    mentions.append(
                        f"<a href='tg://user?id={user.id}'>{utils.esc(user.first_name)}</a>")

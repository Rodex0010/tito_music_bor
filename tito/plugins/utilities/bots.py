...
@app.on_message(filters.command(["البوتات", "bots"], prefixes=["", "/"]) & (filters.group | filters.channel))
@lang.language()
async def list_bots(client, message: Message):
    ...
    try:
        ...
        async for member in client.get_chat_members(message.chat.id, filter=ChatMembersFilter.BOTS):
            ...
            bot_list.append(f"{bot_count}. <a href='tg://user?id={member.user.id}'>{utils.esc(member.user.first_name)}</a> - {bot_username}")
        ...
        # Format the response
        response = message.lang["bots_title"].format(utils.esc(message.chat.title))

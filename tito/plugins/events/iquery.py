...
from tito import app, config, yt
...
async def inline_query_handler(_, query: types.InlineQuery):
    ...
    try:
        ...
        for video in results["entries"]:
            ...
            caption = (
                f"<b>Title:</b> <a href='{link}'>{utils.esc(title[:250])}</a>\n\n"
                f"<b>Duration:</b> {duration}\n"
                f"<b>Views:</b> <code>{views}</code>\n"
                f"<b>Channel:</b> <a href='{channellink}'>{utils.esc(channel)}</a>\n"
                f"<u><i>Fetched by {utils.esc(app.name)}</i></u>"
            )

...
from tito import tune, app, config, db, lang, logger, queue, tg, yt
from tito.helpers import admin_check, buttons, can_manage_vc
...
def safe_callback(func):
    ...
    async def wrapper(client, query: types.CallbackQuery):
        try:
            ...
            try:
                await query.answer("❌ حدث خطأ ما، حاول مرة أخرى.", show_alert=True)
            except Exception:
                pass
    return wrapper
...
async def _controls(_, query: types.CallbackQuery):
    ...
    if not has_permission:
        return await query.answer("⚠️ ليس لديك صلاحية استخدام هذا.", show_alert=True)
    ...
    if action == "pause":
        ...
        # Avoid racing an already in-flight track transition for this chat
        # (see skip.py for why - overlapping calls could invalidate each
        # other and leave nothing playing).
        if tune.get_lock(chat_id).locked():
            return await query.answer("⏳ جاري التحميل بالفعل، لحظات...", show_alert=True)
    ...
    try:
        if action in ["skip", "replay", "stop"]:
            ...
            # The now-playing panel is always sent as a photo (see
            # calls.py:_send_photo_with_retry), so it only ever has a
            # caption, never `.text`. Editing it with edit_message_text
            # fails on media messages (Telegram requires editing the
            # caption instead), which was silently swallowed below and
            # made the panel look like it "disappeared" on pause/resume.
            mtext = re.sub(
        ...
        # Handle FloodWait on edit_message_caption
        await asyncio.sleep(e.value)
        try:
            await query.edit_message_caption(
                caption=f"{mtext}\n\n<blockquote>{reply}</blockquote>", reply_markup=keyboard
...
async def handle_seek(query: types.CallbackQuery, chat_id: int, action: str, user: str):
    ...
    if not media or media.is_live:
        return await query.answer("⚠️ لا يمكن التقديم/الترجيع في البث المباشر!", show_alert=True)
    
    if not media.duration_sec or media.duration_sec == 0:
        return await query.answer("⚠️ لا يمكن التقديم/الترجيع في هذا المقطع!", show_alert=True)
    ...
    if action == "seek_back_10":
        ...
        return await query.answer("⚠️ إجراء تقديم/ترجيع غير صالح!", show_alert=True)

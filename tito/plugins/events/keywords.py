...
#   - Someone writes "نونه"   -> fill NOUNA_REPLIES yourself below
...
BOT_REPLIES = [
    ...
    "<blockquote><b>اسمي نونه الثغنونه 👀</blockquote>",
]

# --------------------------------------------------------------------------
# "نونه" keyword -> fill in your own replies here (same format as the
# pools above: each entry wrapped in <blockquote>...</blockquote>, one is
# picked at random per trigger).
# --------------------------------------------------------------------------
NOUNA_REPLIES = [
    "<blockquote>قلب النونه يناس 🫶🏻</blockquote>",

    "<blockquote><b>قلبهااا 👀🫀</blockquote>",

    "<blockquote><b>ييوووحيي 😂</blockquote>",

    "<blockquote><b>قلبهااا مووووه 💋</blockquote>",

    "<blockquote><b>يعمريي  🫶🏻</blockquote>",
]
...
@app.on_message(
    filters.text
    & filters.regex(r"(?i)\bنونا\b")
    & ~filters.bot
    & ~filters.via_bot
    & ~app.bl_users,
    group=44,
)
async def _nouna_keyword(_, m: types.Message):
    if not NOUNA_REPLIES:
        return  # add your replies to NOUNA_REPLIES above first
    try:
        await m.reply_text(random.choice(NOUNA_REPLIES), quote=True)
    except Exception:
        pass

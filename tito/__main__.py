...
from tito import (tasks, tune, app, azan, config, db,
                   logger, stop, userbot, yt)
from tito.plugins import all_modules
...
async def main():
    try:
        ...
        # Start the prayer-time (azan) scheduler
        azan.boot()

        # Load all plugins
        for module in all_modules:
            try:
                importlib.import_module(f"tito.plugins.{module}")
        ...
        # Self-heal in the background: confirm the bot is still actually in
        # every stored group/channel (drop stale ones), and classify any
        # chat added before the group/channel split existed - so a restart

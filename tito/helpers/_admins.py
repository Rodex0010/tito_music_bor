...
def _is_channel_post(update: types.Message | types.CallbackQuery) -> bool:
    """A genuine post made directly to a channel (not a discussion-group
    comment) has no `from_user` - Telegram only exposes the channel itself
    as the sender. Since only channel admins can post there in the first
    place, treat these the same way play.py's checkUB already does."""
    if not isinstance(update, types.Message):
        return False
    return (
        update.sender_chat is not None
        and update.sender_chat.id == update.chat.id
        and update.chat.type == enums.ChatType.CHANNEL
    )
...
def admin_check(func):
    ...
    async def wrapper(_, update: types.Message | types.CallbackQuery, *args, **kwargs):
        ...
        # A real post made to a channel (not a discussion-group comment) has
        # no from_user - only channel admins can post there, so allow it.
        if _is_channel_post(update):
...
def can_manage_vc(func):
    ...
    async def wrapper(_, update: types.Message | types.CallbackQuery, *args, **kwargs):
        ...
        # Skip if no user (anonymous group admin - can't be verified)
        if not update.from_user:
            return

...
class Utilities:
    ...
    def esc(self, text) -> str:
        """HTML-escape arbitrary user/API-controlled text (names, chat
        titles, track titles, free-typed messages, ...) before it gets
        interpolated into an HTML-parse_mode string. Without this, a raw
        "<", ">" or "&" in the value breaks Telegram's HTML entity parser
        and the send fails with ENTITY_BOUNDS_INVALID/CANT_PARSE_ENTITIES.
        Non-string input is returned unchanged.
        """
        return html.escape(text) if isinstance(text, str) else text

    def actor_mention(self, m: types.Message) -> str:
        """Mention text for whoever triggered a command. Falls back to the
        channel's (escaped) title for genuine channel posts, which have no
        from_user - mirrors the fallback play.py already uses."""
        if m.from_user:
            return m.from_user.mention
        if m.sender_chat:
            return self.esc(m.sender_chat.title)
        return "Channel"
